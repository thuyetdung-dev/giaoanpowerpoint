...
export function MixedMath({value}:{value:string}){const clean=String(value||"").normalize("NFC").replace(/`\s+/g," ");const parts=clean.split(/(\$[^$]+\$|\\\([\s\S]*?\\\))/g);return <>{parts.map((part,i)=>{const math=part.startsWith("$")&&part.endsWith("$")?part.slice(1,-1):part.startsWith("\\(")&&part.endsWith("\\)")?part.slice(2,-2):null;return math!==null?<MathText key={i} value={math}/>:<span key={i}>{part}</span>})}</>}
